...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	...
	fmt.Println(avgSpeedInMetrSec)
	heightInCm := height / cmInM
	return (walkingCaloriesWeightMultiplier*weight + (math.Pow(avgSpeedInMetrSec, 2)/heightInCm)*walkingSpeedHeightMultiplier*weight) * duration * minInH
}
